...
_Last updated: 2026-04-25 after M12. Update this file at the end of each milestone._
...
| **M11** | **⏳ Nick's flight in 2 days** | First real trip. Nick uses the app on an actual airport run. The /qa pre-M11 sweep already shipped 2 critical fixes (`7a419a0` airport picker, `782d064` trip-start enqueue race). Test-trip toggle (`56a6538`) lets Nick distinguish dogfood trips from the real one. |
...
| (next) | M12 backup.sh — B2 nightly mirror + last-upload marker |
| (next) | M12 preflight — local pg_dump + B2 marker recency checks |
| (next) | M12 restore-drill.sh — isolated postgres restore + row-count diff |
| (next) | M12 docs/RECOVERY.md — from-scratch recovery runbook |
...
2. M11 is the only outstanding milestone before M13 — "first real trip." Nick's flight is in 2 days from 2026-04-25. Code-side everything is ship-ready (M10 predict + M12 backups + 2 /qa-found bug fixes + test-trip toggle). M13 (calibration UI) is gated on M11 generating real-trip data.
