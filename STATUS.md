...
_Last updated: 2026-04-27 after M11 (first real round-trip ORD↔LGA, complete). Update this file at the end of each milestone._
...
| M11 | ✅ | First real trip — round-trip ORD↔LGA on 2026-04-27, both legs complete, no voids. Departure: in_transit 12:04 → at_airport 12:56 (52m drive) → bags 13:00 → security 13:05 (9m airport-side). Arrival: off_plane 16:52 → bags 17:02 → in_transit 17:06 (14m airport-side) → at_destination 17:41 (35m drive). Two rough edges Nick flagged from the SkyClub became `5fc45f8` polish: (1) Predict gains optional **scheduled landing** date+time pair (departure-only), flowing through `window.__predictHandoff`/`saved-predictions` into the dep trip-start sheet's existing sched-arr inputs so fewer fields get retyped; (2) standalone `<input type="time"\|"date">` in `.form-row` had no CSS rule (only `.datetime-row input` was styled), making the Scheduled Boarding inputs render invisible against the dark background — fixed in style.css. Initial in-toast predicted-vs-actual line was attempted but read-poorly mid-trip → replaced in `356dad5` with an **inline "Last legs" panel** above the active grid: one row per completed segment (drive, airport) showing actual + typical p50–p90 + verdict (ahead ✓ / on-track / running long), accumulating as the trip progresses. Pure client-side — no schema change; M13 calibration scope unchanged. /qa pre-M11 sweep had earlier shipped 2 critical fixes (`7a419a0` airport picker, `782d064` trip-start enqueue race) plus the test-trip toggle (`56a6538`). M14 (below) layered live-drive + segment breakdown + handoff into Predict before the flight so the morning leave-by wasn't pure history. |
...
| M15 | ✅ | Saved predictions — bridge the day-or-more gap between running a prediction and starting the trip. Predict result card grows a **"Save for later"** button next to "Start this trip →"; tapping persists the full handoff payload + the live buffer-baked hero offset to `localStorage` under `travel:saved-predictions`. Log screen empty state surfaces saved entries as a banner above the Dep + Arr heroes (accent left-border, "Leave by 4:30 PM" hero copy + "DEP LGA Mon, Apr 27, 5:19 PM from Home" sub). Tap → resumes through the existing handoff path (same `openDepStartSheet`/`openArrivalStartSheet` calls); × dismisses without resuming. Entries dedupe by (direction, airport, flight_date, flight_time) so re-saving updates in place; Save button reflects "Update saved" or "Saved ✓". Auto-purge on every render: anything with `flight_utc_ms < now - 6h` gets dropped. Pure client-side feature — no schema change, no server work; `web/saved-predictions.js` is the storage module. Empty state switches from `grid-template-rows: 1fr 1fr` to flex-column when banners are present so heroes still split the remaining viewport. |
...
| `5b65b08` | M15 saved predictions — Save for later button + Log empty-state banner + localStorage module |
| `5fc45f8` | M11 polish (mid-trip from SkyClub) — sched landing carry-through, time/date input styling, in-toast predicted-vs-actual (later replaced) |
| `356dad5` | M11 polish part 2 — inline "Last legs" panel above active grid replaces in-toast comparison |
