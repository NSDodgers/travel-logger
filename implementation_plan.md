...
5. **M5: Historical import.** ✅ Done 2026-04-24. 5 sources reconciled (narrative file 2017–2022, april-22-feb-21 CSV, Bronx CSV, Inwood CSVs, Flighty enrichment). 231 trip rows + 536 milestones + 14 addresses loaded. 2 schema additions: party enum 3-way + dep/arr customs kinds. Re-load: `./scripts/load-legacy.sh`. Reference: `docs/M5_INVENTORY.md`, `docs/M5_QUIZ.md`, `docs/M5_QUIZ_2.md`.
6. **M6: Address book + Mapbox.** ✅ Done 2026-04-25. PWA shell scaffold + hash router + tab bar. Mapbox Search Box REST (suggest+retrieve, no JS lib — keeps `script-src 'self'`) + Static Images pin preview. Address add/edit/archive working end-to-end through PostgREST. 14 legacy labels renamed to human-readable (migration 002). Caddy startup shim generates `/config.js` from the `mapbox_token` secret. Cloudflare cache bypassed for the hostname so iteration loop stays fast. QA driver added: `bun run qa <cmd>` (Playwright persistent context).
7. **M7: Log grid + edit/undo + departure flow.** ⏳ NEXT — see `docs/M7_BRIEF.md`. Dep: In Transit creates trip; subsequent milestones one-tap; long-press edit/void; undo toast. Bag-hiding dynamic via `milestone_kinds.shown_when_carry_on`. Writes direct (no queue yet).
